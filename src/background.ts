...
import GetBilibiliSearchResultService from '@/service/search'
...
ipcMain.handle('search', (event, keyword:string) => {
  return new Promise((resolve, reject) => {
    try {
      const searchService = new GetBilibiliSearchResultService()
      searchService.run(keyword).then(res => {
        resolve(res)
      }).catch(err => {
        reject(err)
      })
